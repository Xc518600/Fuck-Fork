...
if ($.isNode()) {
  ...
  if (process.env.XM_URL && process.env.XM_URL.indexOf('#') > -1) {
  ximeiurl = process.env.XM_URL.split('#');
  console.log(`您选择的是用"#"隔开\n`)
  }
  else if (process.env.XM_URL && process.env.XM_URL.indexOf('\n') > -1) {
   ximeiurl = process.env.XM_URL.split('\n');
   console.log(`您选择的是用换行隔开\n`)
  } else {
   ximeiurl = process.env.XM_URL.split()
  };
  if (process.env.XM_HD && process.env.XM_HD.indexOf('#') > -1) {
  ximeihd = process.env.XM_HD.split('#');
  console.log(`您选择的是用"#"隔开\n`)
  }
  else if (process.env.XM_HD && process.env.XM_HD.indexOf('\n') > -1) {
   ximeihd= process.env.XM_HD.split('\n');
   console.log(`您选择的是用换行隔开\n`)
  } else {
   ximeihd = process.env.XM_HD.split()
