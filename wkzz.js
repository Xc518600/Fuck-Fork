...
const wkzzurlArr = [], wkzzhdArr = [],wkzzidArr = []
...
if (
    process.env.WKZZ_URL &&
    process.env.WKZZ_URL.indexOf(COOKIES_SPLIT) > -1
  ) {
    wkzzurl = process.env.WKZZ_URL.split(COOKIES_SPLIT);
  } else {
    wkzzurl = process.env.WKZZ_URL.split();
  }
  if (
    process.env.WKZZ_HD &&
    process.env.WKZZ_HD.indexOf(COOKIES_SPLIT) > -1
  ) {
    wkzzhd = process.env.WKZZ_HD.split(COOKIES_SPLIT);
  } else {
    wkzzhd = process.env.WKZZ_HD.split();
  }
  if (
    process.env.WKZZ_ID &&
    process.env.WKZZ_ID.indexOf(COOKIES_SPLIT) > -1
  ) {
    wkzzid = process.env.WKZZ_ID.split(COOKIES_SPLIT);
  } else {
    wkzzid = process.env.WKZZ_ID.split();
  }
  ...
  Object.keys(wkzzurl).forEach((item) => {
    ...
    Object.keys(wkzzid).forEach((item) => {
        if (wkzzid[item]) {
          wkzzidArr.push(wkzzid[item])
        }
    });
